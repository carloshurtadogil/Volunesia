...
namespace Volunesia.Droid
{
    ...
    public class SecondRegisterActivity : Activity, IOnCompleteListener
    {
        ...
        public void ValidateRegistrationInformation(object sender, EventArgs e)
        {
            ...
            bool passwordLengthSuccess = true;
            ...
            if (!credVerification.CheckIfPasswordIsValid(password.Text))
            {
            ...
            if (passwordSuccess && passwordLengthSuccess && emailSuccess)
            {
            ...
            else
            {
                ...
                if (!passwordLengthSuccess)
                {
                    errorMessage.Append("Password length must be at least 8 characters long");
                }
                if (!emailSuccess)
                {
                    errorMessage.Append("Email address is not in proper format");
                }



                AlertDialog.Builder dialogAlertConstruction = new AlertDialog.Builder(this);
                dialogAlertConstruction.SetTitle("Password/Email Error");
                dialogAlertConstruction.SetMessage(errorMessage.ToString());
